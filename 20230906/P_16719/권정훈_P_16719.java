...
public class test {
	...
	private static void zoac(int st, int ed) {
		...
		// 분할정복 수행
		// 맨 처음 함수를 호출할 때 입력문자열에서 사전 기준으로 가장 빨리 오는 문자가 선택되었으므로,
		// 해당 문자는 이후의 모든 부분문제를 푸는 과정에서 이미 가장 빠른 문자이기 때문에,
		// 오른쪽 부분문제를 먼저 해결하여 처음에 찾아낸 문자가 최대한 앞에 오게 한 뒤에,
		// 이후 왼쪽 부분문제를 해결하여 정답을 도출해야 한다.
