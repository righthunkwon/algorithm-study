...
public class BOJ_Q17215_볼링점수계산 {
	public static void main(String[] args) {
		...
		int idx =0;
		//기본 핀 개수 배열에 넣기(단, 스트라이크치면 10, -1을 넣어줌(칸수 맞추기 위해))
		...
		//총점 계산
		int total=0;
		for(int i=0;i<20;i++) { //20번째부터는 스페어,스트라이크 보너스 기록은 되지만 더하지 않으면 됨
			if(pin[i]>=0)total+=pin[i]; //스트라이크 뒤에 임의로 넣어둔 -1은 더하면 안되니까
			total+=spareBonus[i];
			total+=strikeBonus[i];
		}
